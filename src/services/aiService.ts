...
const MEMORY_CONTEXT_MAX_LINES = 3;
...
function resolveUserId(options: AnswerOptions): string | undefined {
  const metadataUserId = options.contextMetadata?.userId;
...
async function fetchRecentUserMemories(userId: string, limit: number): Promise<StoredMemory[]> {
  try {
    const collectionRef = db.collection(MEMORY_COLLECTION) as any;
    ...
    console.warn(`[aiService] No fue posible cargar memorias para ${userId}:`, error);
    return [];
  }
}

function buildMemorySystemContext(memories: StoredMemory[]): string | undefined {
  if (!memories.length) {
    return undefined;
  }

  const limited = memories.slice(0, MEMORY_CONTEXT_MAX_LINES);
  const lines = limited.map((memory) => {
    const label = memory.type.replace(/_/g, ' ');
    return `- ${label}: ${memory.text}`;
  });

  const guidance = 'Personaliza la respuesta respetando estas memorias cuando aplique.';
  const contextLines = ['Memorias recientes del usuario:', ...lines];

  if (contextLines.length < 3) {
    contextLines.push(guidance);
  } else if (contextLines.length === 3) {
    contextLines.push(guidance);
  } else if (contextLines.length === 4) {
    contextLines.push(guidance);
  }

  return contextLines.slice(0, 5).join('\n');
}

async function storeNewMemories(
  userId: string,
  text: string,
  existingMemories: StoredMemory[],
): Promise<void> {
  const candidates = extractMemoryCandidates(text).filter(shouldRemember);
  if (!candidates.length) {
    return;
  }

  const existingFingerprints = new Set(
    existingMemories.map((memory) => memoryFingerprint(memory.type, memory.text)),
  );
  const batch = db.batch();
  let hasWrites = false;

  for (const candidate of candidates) {
    const fingerprint = memoryFingerprint(candidate.type, candidate.text);
    if (existingFingerprints.has(fingerprint)) {
      continue;
    }

    const docRef = db.collection(MEMORY_COLLECTION).doc();
    batch.set(docRef, {
      userId,
      text: candidate.text,
      type: candidate.type,
  ...
  try {
    ...
    console.warn(`[aiService] No fue posible guardar memorias para ${userId}:`, error);
  }
}
...
export async function answerWithPromptBase(options: AnswerOptions): Promise<AnswerResult> {
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt += 1) {
    try {
      ...
      if (userId) {
        await storeNewMemories(userId, `${options.userMessage}\n${finalText}`, recentMemories);
      }
